...
function virusCheckRecursive(str, viruses) {
  ...
  let bw = str.toLowerCase();
  let vrs = viruses.toLowerCase()
if(vrs[0] == '|') return virusCheckRecursive(bw, vrs.slice(1))
  let hasil = 0
  for(let i=0;i<bw.length;i++){
    if(vrs[0] == bw[i]) hasil += 1
  }
  return hasil + virusCheckRecursive(bw, vrs.slice(1))
}
